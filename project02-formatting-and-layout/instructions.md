...
- The goal of a container is to *keep any paragraph* from extending any wider than say, 600px or so, which would be a readability problem.  
  - Note: you must *not* constrain *all* the content within 600px.  (That would be a waste of space.)  Just make sure that no <u>paragraph</u> of text is allowed to get any wider than that.
